const { createCloze } = require("./cloze.js");
const pangu = require("pangunode");
const { getWord } = require("./dict.js");

function parseSectionConfig(line) {
    const filedType = {};
    line.replace(":", "")
        .trim()
        .split(" ")
...
function fileParse(text) {
    ...
    while (count < tokens.length) {
        ...
        if (/^@.*?:$/.test(line)) {
            Object.assign(globalProps, parseSectionConfig(line));
        } else if (/^- /.test(line)) {
            let front = line.replace("- ", "");
            const noteProps = parseSectionConfig(line);
